...
if (!eventId) {
    document.getElementById("event-title").innerText = "Lipsește codul evenimentului din link.";
} else {
    loadEvent();
    listenToLeaderboard();
}
...
function loadEvent() {
    ...
    .then((doc) => {
        ...
        const data = doc.data();
        document.getElementById("event-title").innerText = `Eveniment: ${data.name}`;
    })
    .catch((error) => {
        console.error(error);
    });
...
function submitData() {
    ...
    if (alcohol < 0 || alcohol > 5) {
        alert("Introdu o valoare validă.");
        return;
    }
    ...
    db.collection("events")
      ...
      .then(() => {
          ...
          document.getElementById("statusMessage").innerText = "Rezultatul a fost adăugat.";
      })
      .catch((error) => {
